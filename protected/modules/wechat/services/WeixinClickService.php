...
class WeixinClickService {
    ...
    public static function getLoginUrl($obj, $user) {
        ...
        $result = Users::model()->updateByPk($user->user_id, array("repstaken" => $mdstring, "repsativetime" => time()));
        if ($result) {
            $content = '请在10秒内进入>> <a href="http://www.zuanzuanle.com/wechat/help/login/id/' . $user->user_id . '/stoken/' . $mdstring . '.html">首页</a>,否则授权将失效。';
        } else {
    ...
    public static function getAccountClick($object, $weixinuser) {
        $myAccount = Account::model()->find("user_id=:user_id", array(":user_id" => $weixinuser->user_id));
